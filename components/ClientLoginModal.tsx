...
export const ClientLoginModal: React.FC<ClientLoginModalProps> = ({ isOpen, onClose, language }) => {
    ...
    const handleGoogleLoginCustom = useGoogleLogin({
        onSuccess: async (tokenResponse) => {
            console.log("[Google Auth] Custom Token Response Received");
            if (!acceptedTerms) {
            ...
            try {
                ...
                console.log("[Google Auth] User Info Fetched:", userInfo.email);
                ...
                setError(err.message || 'Erro ao processar login do Google.');
            } finally {
                setLoading(false);
            }
        },
        onError: (error) => {
            ...
            setError('Falha na autenticação do Google. Tente novamente.');
        }
    });
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex justify-center w-full">
                                            <button
                                                ...
                                                disabled={loading}
                                                className="flex items-center justify-center gap-3 w-full max-w-[300px] bg-white border border-gray-300 text-gray-700 py-3 px-4 rounded-full font-medium hover:bg-gray-50 transition-all shadow-sm active:scale-[0.98]"
                                            >
